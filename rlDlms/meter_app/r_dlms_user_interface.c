...
Integer32 R_USER_GetEMData(Unsigned16 em_data)
{
	...
	switch(em_data)
	{
		...
		case EM_BLOCK_AVG_PF:
			result= InsSave.kVAhCounter10m;
			result=InsSave.lskVAhPulseCounter*10+result/METER_CONSTANT;
			if(result>0)
				result = (InsSave.lskWhPulseCounter*10+InsSave.kWhCounter10m/METER_CONSTANT)*100/result;
			else
				result=0;
		...
		case EM_DEMAND_INTEGRATION_PERIOD:
			...
			if(md_period!=new_md_period)
				result = 3600/new_md_period;
			else
		...
		case EM_BLOCK_ENERGY_KWH:
			result=InsSave.kWhCounter10m;
			result = InsSave.lskWhPulseCounter+result/METER_CONSTANT;
			break;
		...
		case EM_BLOCK_ENERGY_KVAH:
			result=InsSave.kVAhCounter10m;
			result =InsSave.lskVAhPulseCounter+result/METER_CONSTANT;
			break;
		...
		case EM_BLOCK_AVG_CURRENT:
			...
			if(InsSave.PowerOn30>0)
				...
				if(result)
				{
					result=InsSave.lsAvgPh_EffC/result;
				}
			#endif
			break;
		...
		case EM_BLOCK_AVG_NU_CURRENT:
			...
			if(InsSave.PowerOn30>0)
				...
				result=InsSave.timeCounter%(3600/ls_period);
				if(result)
					result=InsSave.lsAvgNuC/result;
			#endif
...
)
{
	...
	// progess in reverse way
	//Neu Current,Effective I/Ph Current, kVAh, kWh ,avg voltage
	addr=5;
	...
	if(addr>5)
	{
		lsVal=getByteValue(lsBuffer, 5, addr-5);
		lsVal<<=8;
	}
	lsVal|=getByteValue(lsBuffer, 4, 1);
	lsVal<<=32;
	lsVal|=getByteValue(lsBuffer, 0, 4);
	...
	#if (defined(IS_NU_I_AVG) && (IS_NU_I_AVG == 1))
		buf->NuCuurent=lsVal&0x0FFF;
		buf->NuCuurent=buf->NuCuurent*10;
		lsVal>>=12;
		#if ((defined(IS_EFF_I_AVG) && (IS_EFF_I_AVG == 1))||(defined(IS_PHASE_I_AVG) && (IS_PHASE_I_AVG == 1)))
			buf->PhEffCurrent=lsVal&0x0FFF;	// Ph Current of Effective Current
			buf->PhEffCurrent=buf->PhEffCurrent*10;
			lsVal>>=12;
		#endif
	#else
		#if ((defined(IS_EFF_I_AVG) && (IS_EFF_I_AVG == 1))||(defined(IS_PHASE_I_AVG) && (IS_PHASE_I_AVG == 1)))
		buf->PhEffCurrent=lsVal&0xFFFF;	// Ph Current of Effective Current
		buf->PhEffCurrent=buf->PhEffCurrent*10;
		lsVal>>=24;
		#endif
	#endif
	...
	buf->blockkVAh_value =(lsVal&0xFFFF); // block kVAh
	lsVal>>=16;
	#else
	buf->blockkVAh_value =(lsVal&0x0FFF); // block kVAh
	lsVal>>=12;
	#endif
	
	
	buf->blockkWh_value = (lsVal&0xFFFF);// Block kWh
	lsVal>>=16;
	buf->voltage_value = (lsVal&0xFF)*200;// Block AvgV
	lsVal>>=8;
	
	buf->block_demand=buf->blockkWh_value*ls_period;
	//-------------------------
	if(buf->blockkVAh_value)
		buf->blockPf=buf->blockkWh_value*100/buf->blockkVAh_value;
	else
		buf->blockPf=0;
	if(buf->blockPf>100)
		buf->blockPf=100;
...
)
{
	...
	if((index+1)==g_Class07_Billing_EntriesInUse)
	{
		...
		#if (defined(IS_MD_2DECIMAL) && (IS_MD_2DECIMAL == 1))
			buf->active_MD=buf->active_MD/10;
			buf->apparent_MD=buf->apparent_MD/10;	
		#endif
		...
		while(max_zone_no--)
		{
			...
			#if (defined(IS_MD_2DECIMAL) && (IS_MD_2DECIMAL == 1))
				buf->active_TZ[max_zone_no].MD=buf->active_TZ[max_zone_no].MD/10;
				buf->apparent_TZ[max_zone_no].MD=buf->apparent_TZ[max_zone_no].MD/10;
			#endif

		}
		buf->tamper_count=getTotalTamperCount();
	}
	else
	{
		...
		buf->tamper_count=getByteValue(billBuffer, 40, 2);
...
)
{
	...
	buf->nu_current_value = getByteValue(tBuffer, 10, 4);
	...
	buf->eff_current_value = getByteValue(tBuffer, 21, 4);
	return 0;
}
...
Unsigned16 R_OBIS_Class07_GetInstantEntry(class07_instant_entry_t *buf)
{
	...
	buf->selfd = R_USER_GetEMData(EM_INTERNAL_DIAGNOSTICS);
	buf->billday = R_USER_GetEMData(EM_PRE_BILL_DATE_READ);
...
)
{
	...
	uint16_t ee_addr = week_loc[is_active] + (index * 23);
	
	read_eeprom(buf->p_week_profile_name, ee_addr, name_len);
	
	ee_addr+=8;
	read_eeprom(&buf->Monday, ee_addr, 7);
...
Unsigned8 R_USER_Class20_setTariffWeekTable(
	week_profile_t      *buf,
	Unsigned16          index,
	Unsigned16          is_active
)
{
	...
	write_eeprom(buf->p_week_profile_name, ee_addr, name_len);
	
	ee_addr+=8;
	write_eeprom(&buf->Monday, ee_addr, 7);
