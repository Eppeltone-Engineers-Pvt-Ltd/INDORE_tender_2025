...
{   
	...
	RxTxBuffer[29]=resetType;       //reset type                  //1 byte
		...
		#endif
		makeByte(TamperRecord.MonthTamperStatus,38,2);	//2 bytes tamper status
		makeByte(getTotalTamperCount(),40,2);	//2 bytes tamper status
		...
		RxTxBuffer[41]=resetType;       //free bytes    //1 byte
	#endif
