...
void updateEnergyPulsePara(uint8_t ioption,uint32_t CurrentVal);
				...
				if(mcu_flag&DO_MEASUREMENT)
				{
					...
					if((Ins.EffectiveI>0)&&(GET_MAINS_STATUS==0))
					{
						...
						InsSave.PowerOn30+=(TRTCMP0/256);
							...
							InsSave.lsAvgPh_EffC=InsSave.lsAvgPh_EffC+Ins.EffectiveI*(TRTCMP0/256);
						#elif (defined(IS_PHASE_I_AVG) && (IS_PHASE_I_AVG == 1))
							InsSave.lsAvgPh_EffC=InsSave.lsAvgPh_EffC+Ins.PhCurrent*(TRTCMP0/256);
						#else
							#error "avg current not defined."
						#endif
						
						#if (defined(IS_NU_I_AVG) && (IS_NU_I_AVG == 1))
							InsSave.lsAvgNuC=InsSave.lsAvgNuC+Ins.NeuCurrent*(TRTCMP0/256);
						#endif						
						...
						#if (defined(IS_ADD_NM_CURRENTS) && (IS_ADD_NM_CURRENTS == 1))
							updateEnergyPulsePara(TRTCMP0/256,Ins.NeuCurrent+Ins.PhCurrent);
						#else
							updateEnergyPulsePara(TRTCMP0/256,Ins.EffectiveI);
						#endif
						...
						{
							if(is_vfail_with_load==0)
								updateEnergyPulsePara(TRTCMP0/256+2,Ins.EffectiveI);
						...
						if(op_action_state&PFAIL_MODE_PULSE)
						{
							...
							else
							{
								doMeasurement_NM(1);
								#if (defined(IS_ADD_NM_CURRENTS) && (IS_ADD_NM_CURRENTS == 1))
									activePulsePower=REFERENCEVOLTAGE*(Ins.NeuCurrent+Ins.PhCurrent);
								#else
...
void updateEnergyPulsePara(uint8_t mVal,uint32_t CurrentVal)
{
	apperentPowerNM=REFERENCEVOLTAGE*CurrentVal*mVal;
